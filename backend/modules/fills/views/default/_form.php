...
<div class="row">
    ...
    <div class="col-md-4"><?= $form->field($model, 'plate_id')->dropDownList(
        $plateItems,
        $searchableOptions + ['prompt' => Yii::t('app', '选择车牌')]
    ) ?></div>
    <div class="col-md-4"><?= $form->field($model, 'counter_id')->dropDownList(
        ArrayHelper::map(GslCounter::find()->orderBy('sort_order')->all(), 'id', 'code'),
...
(function () {
    ...
    function refreshPlates(keepSelection) {
        ...
        if (window.GslSearchableSelect) {
            GslSearchableSelect.refresh(\$plate);
        }
    }
    ...
    refreshPlates(true);
    if (window.GslSearchableSelect) {
        GslSearchableSelect.bind('#gslfill-company_id, #gslfill-plate_id, #gslfill-counter_id', emptyText);
    }
})();
JS;
$this->registerJs($js);
